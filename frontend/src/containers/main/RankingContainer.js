  ...
  useEffect(() => {
    if (ranking) {
      setRank((prevState) => {
        //  initialize
        if (!prevState) {
          return ranking.map((ranking, index) => ({
            ...ranking,
            index,
          }));
        }
        ...
        const revisedState = prevState
          ...
          .map((internalCell) => {
            const index = ranking.findIndex(
              (externalCell) => externalCell.username === internalCell.username
            );
            if (index >= 0) {
              stable.push(index);
              // eslint-disable-next-line
              if (
                JSON.stringify({ ...ranking[index], index }) ==
                JSON.stringify(internalCell)
              ) {
                return internalCell;
              }
              return {
          ...
          setTimeout(() => {
            setRank((prevState) => {
              ...
              newlyPushed.forEach((externalCell) => {
                const index = copiedRanking.findIndex(
                  (internalCell) =>
                    internalCell.username === externalCell.username
                );
                if (index) {
