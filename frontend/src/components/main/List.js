...
const ListCell = React.memo(({ cell }) => {
  return (
    <ListCellBlock index={cell.index}>
      <TextBlock>{cell.index + 1}</TextBlock>
      <TextBlock mail>{cell.username}</TextBlock>
      <TextBlock elo>{cell.elo}</TextBlock>
      <TextBlock>{cell.ratio}</TextBlock>
      <TextBlock>{cell.win}</TextBlock>
      <TextBlock>{cell.lose}</TextBlock>
    </ListCellBlock>
  );
});
...
const HeaderCell = React.memo(() => {
  return (
    <HeaderCellBlock>
      ...
      <TextBlock elo={true}>Elo</TextBlock>
...
const List = ({ list }) => {
  const listArr =
    list && list.map((cell) => <ListCell key={cell.username} cell={cell} />);
  ...
  if (list) {
    ...
    for (let i = 1; i < leng; ++i) {
      borderArr.push(<BorderBlock key={`border_${i}`} index={i} />);
    }
  }
